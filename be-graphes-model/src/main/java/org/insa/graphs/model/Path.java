...
public class Path {
	...
	public boolean isValid() {
		/*
		 * TODO: Check if this path is valid. A path is valid if any of the following is
		 * true:
		 * 
		 * it is empty; it contains a single node (without arcs); the first arc has for
		 * origin the origin of the path and, for two consecutive arcs, the destination
		 * of the first one is the origin of the second one.
		 */
		if (this.isEmpty() || this.arcs.size() <= 1)
			return true;
		...
		/*
		 * if // this.getArcs().origin==this.Path and ...
		 * 
		 */
	...
	public float getLength() {
		...
		// TODO: SOMMATION DES TAILLES DES ARCS
		for (int i = 0; i < this.arcs.size(); i++) {
			sum += this.arcs.get(i).getLength();
		}
	...
	public double getTravelTime(double speed) {
		// TODO:Recupérer le temps de trajet de chaque arc sommé
		double sum = 0;

		for (int i = 0; i < this.arcs.size(); i++) {
			sum += this.arcs.get(i).getTravelTime(speed);
		}
		return sum;
	}
	...
	public double getMinimumTravelTime() {
		...
		for (int i = 0; i < arcs.size(); i++) {
			sum += this.arcs.get(i).getMinimumTravelTime();
		}
		return sum;
	}
